...
contract SolutionBankUnitTest is Test {
    Bank bank;
    ...
    function test_ownerClose() public {
        ...
        assertFalse(bank.isOpened(), "not closed");
        assertEq(bank.OWNER(), OWNER, "invalid owner");
    }
    ...
    function test_depositExceedMaxBalance() public {
        uint256 amount = bank.maxBalance() - IERC20(token).balanceOf(address(bank)) + 1;
        // add token to alice
        deal(token, ALICE, amount);
        vm.startPrank(ALICE);
        IERC20(token).approve(address(bank), amount);
        vm.expectRevert("exceed max balance");
        bank.deposit(amount);
        vm.stopPrank();
    }
    ...
    function test_withdrawSimple() public {
        ...
        assertEq(bankBalBf - IERC20(token).balanceOf(address(bank)), amount, "invalid bank balance");
        assertEq(aliceBankBalBf - bank.balances(ALICE), amount, "invalid bank balance");
    }
