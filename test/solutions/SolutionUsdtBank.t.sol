...
contract SolutionUsdtBankTest is Test {
    ...
    function test_fuzzWithdraw(uint256 amount, uint256 withdrawAmount) public {
        amount = bound(amount, 1, bank.maxBalance() - IERC20(token).balanceOf(address(bank)));
        deal(token, ALICE, amount);
        // prank alice
        vm.startPrank(ALICE);
        ...
        uint256 bankBalBf = IERC20(token).balanceOf(address(bank));
        uint256 aliceBankBalBf = bank.balances(ALICE);
        withdrawAmount = bound(withdrawAmount, 1, bank.balances(ALICE));
        // time skip
        skip(1 days);
        // prank alice
        vm.startPrank(ALICE);
        ...
        vm.stopPrank();
        // check the balance of bank is correct
        assertEq(IERC20(token).balanceOf(ALICE) - aliceBalBf, withdrawAmount, "invalid alice balance");
        assertEq(bankBalBf - IERC20(token).balanceOf(address(bank)), withdrawAmount, "invalid bank balance");
        assertEq(aliceBankBalBf - bank.balances(ALICE), withdrawAmount, "invalid bank balance");
    }
    ...
    function test_twoUserDepositAliceWithdraw() public {
        ...
        skip(1 days);
        vm.startPrank(ALICE);
        bank.withdraw(aliceAmount);
        vm.stopPrank();
        // check the balance of bank is correct
        assertEq(bankBalBf - IERC20(token).balanceOf(address(bank)), aliceAmount, "invalid bank balance");
        // check the balance of alice is correct
        assertEq(IERC20(token).balanceOf(ALICE) - aliceBalBf, aliceAmount, "invalid alice balance");
        // check the alice's balance in bank is correct
        assertEq(aliceBankBalBf - bank.balances(ALICE), aliceAmount, "invalid bank balance");
        // check that bob's balance is not affected
        assertEq(bobBalBf, IERC20(token).balanceOf(BOB), "invalid bob balance");
        // check that bob's balance in bank is not affected
        assertEq(bobBankBalBf, bank.balances(BOB), "invalid bank balance");
    }
